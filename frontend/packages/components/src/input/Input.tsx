    ...
    >
      ...
      <div
        className={mergeClasses(
          "tw-relative tw-flex tw-flex-col tw-w-full",
          label && "tw-mt-3.5"
        )}
      >
        {label && (
...
// TODO: use ref
export const DropdownInput: React.FC<DropdownInputProps> = forwardRef(
  (props, ref) => {
    ...
    const getElementForDisplay = props.getElementForDisplay
      ? props.getElementForDisplay
      : (value: any) => value;
    const showLabel =
      props.label !== undefined && (open || props.value !== undefined);
...
const DropdownOptions: React.FC<DropdownOptionsProps> = (props) => {
  ...
  if (props.options && props.options.length > 0) {
    ...
    return (
      <div className="tw-p-2 tw-pl-4 tw-select-none">
        {props.noOptionsString}
      </div>
    );
  }
...
// TODO: use ref
export const ComboInput: React.FC<ComboInputProps> = forwardRef(
  (props, ref) => {
    ...
    const getFilteredOptions = () => {
      ...
      if (props.options) {
        return props.options.filter((option) => {
          const displayValue = props.getElementForDisplay
            ? props.getElementForDisplay(option)
            : option;
          ...
          >
            <div className="tw-py-3.5 tw-px-3 tw-flex tw-flex-1">
              <Combobox.Input
                ...
                displayValue={(value) =>
                  value ? getElementForDisplay(value) : ""
                }
                onChange={(event) => setQuery(event.target.value)}
                placeholder={props.placeholder}
              />
              <Combobox.Button
                className="tw-inline-block tw-h-full"
                ref={buttonRef}
              >
                <span className="tw-pointer-events-none pr-2">
                  <ChevronUpDownIcon
...
export const PhoneInput = forwardRef<
  HTMLInputElement,
  InputProps & { wrapperClass: string }
>((props, ref) => {
  ...
  const updatePhoneNumber = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onChange &&
      onChange({ ...event, target: { ...event.target, value: formatted } });
  };
...
export const PriceInput = forwardRef<HTMLInputElement, InputProps>(
  (props, ref) => {
    ...
    const stringifyPrice = (
      price: number | undefined | string | readonly string[]
    ): string => {
                    ...
                    <div className="tw-flex tw-items-center">
                      {props.noBullet || (
                        <span
                          className={mergeClasses(
                            checked
                              ? "tw-bg-slate-600 tw-border-transparent"
  ...
  return (
    <>
      ...
      >
        ...
        <span className="tw-flex tw-flex-grow tw-justify-center tw-select-none">
          {value}
        </span>
      </button>
      <Transition
        ...
        <FloatingFocusManager context={context}>
          ...
          >
            <div className="tw-flex tw-justify-between">
              <span className="tw-whitespace-nowrap tw-select-none">
                Adults
              </span>
              <div className="tw-flex tw-gap-3">
                ...
                <span className="tw-flex tw-w-3 tw-justify-center tw-select-none">
                  {value}
                </span>
                <button
                  onClick={() => {
                ...
                >
                  <PlusCircleIcon
                    className={mergeClasses(
                      "tw-w-6 tw-cursor-pointer tw-stroke-gray-500 hover:tw-stroke-black",
                      value === maxGuests &&
                        "!tw-stroke-gray-300 tw-cursor-not-allowed"
                    )}
                  />
                </button>
              </div>
