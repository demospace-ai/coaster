...
export const Input: React.FC<ValidatedInputProps> = (props) => {
	...
	let classes = [
		"tw-border tw-border-solid tw-border-slate-300 tw-rounded-md tw-py-2.5 tw-px-3 tw-w-full tw-box-border hover:tw-border-slate-400 focus:tw-border-slate-700 tw-outline-none",
		props.className,
	];
...
export const ValidatedInput: React.FC<ValidatedInputProps> = (props) => {
	...
	const showLabel =
		label !== undefined &&
		(focused || (value !== undefined && `${value}`.length > 0));
...
export const ValidatedDropdownInput: React.FC<ValidatedDropdownInputProps> = (
	props
) => {
	const [isValid, setIsValid] = useState(true);
	const [focused, setFocused] = useState(false);
	...
	const getElementForDisplay = props.getElementForDisplay
		...
		: (value: any) => value;
	const getElementForDropdown = props.getElementForDropdown
		? props.getElementForDropdown
		: getElementForDisplay;
	const showLabel =
		props.label !== undefined && (focused || props.selected !== undefined);
			...
			<div className="tw-relative tw-w-fit">
				...
				<Listbox.Button
					...
					className={mergeClasses(
						...
						props.className,
						props.validated && !isValid && "tw-border-red-600"
					)}
				>
					<div
						className={mergeClasses(
					...
					>
						{value !== UNSET
							? getElementForDisplay(props.selected)
							: props.placeholder}
					</div>
					{!props.noCaret && (
...
export const ValidatedComboInput: React.FC<ValidatedComboInputProps> = (
	props
) => {
	const [isValid, setIsValid] = useState(true);
				...
				>
					<Combobox.Input
						...
						displayValue={(selected) =>
							selected !== UNSET ? getElementForDisplay(selected) : ""
						}
