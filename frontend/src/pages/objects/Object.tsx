  ...
  return (
    <div className="tw-pt-5 tw-pb-24 tw-px-10 tw-h-full tw-w-full tw-overflow-scroll">
      ...
      <div className="tw-flex tw-flex-col tw-flex-wrap tw-items-start tw-h-32 tw-w-1/2 tw-p-4 tw-mb-5 tw-bg-white tw-border tw-border-slate-200 tw-rounded-md">
        ...
        <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
          <span className="tw-font-medium tw-whitespace-pre">Target Type: </span>{targetTypeToString(object.target_type)}
        </div>
        {object.target_type !== TargetType.Webhook &&
          <>
            <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
              <span className="tw-font-medium tw-whitespace-pre">Namespace: </span>{object.namespace}
            </div>
            <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
              <span className="tw-font-medium tw-whitespace-pre">Table Name: </span>{object.table_name}
            </div>
          </>
        }
        {needsCursorField(object.sync_mode) &&
          <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
            <span className="tw-font-medium tw-whitespace-pre">Cursor Field: </span>{object.cursor_field}
          </div>
        }
        {needsPrimaryKey(object.sync_mode) &&
          <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
            <span className="tw-font-medium tw-whitespace-pre">Primary Key: </span>{object.primary_key}
          </div>
        }
        {needsEndCustomerId(object.target_type) &&
          <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
            <span className="tw-font-medium tw-whitespace-pre">End Customer ID Field: </span>{object.end_customer_id_field}
          </div>
        }
        <div className="tw-flex tw-flex-row tw-items-center tw-text-base tw-mt-1">
          <span className="tw-font-medium tw-whitespace-pre">Frequency: </span>{object.frequency} {object.frequency_units}
        </div>
      </div>

      <div className="tw-font-bold tw-text-lg tw-mt-10 tw-mb-2">
        Object Fields
      </div>
        ...
        <table className="tw-min-w-full tw-border-spacing-0 tw-divide-y tw-divide-slate-200">
          ...
          <tbody className="tw-divide-y tw-divide-slate-200 tw-bg-white">
            {object.object_fields.map((objectField, index) => {
              return <tr key={index} className="tw-cursor-pointer hover:tw-bg-slate-50">
