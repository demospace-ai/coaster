...
const AssigneeInput: React.FC<AssigneeInputProps> = props => {
  ...
  return (
    <div className="tw-top-16 tw-w-48">
      <Combobox value={props.assignee} onChange={props.setAssignee} nullable>
        <div className="tw-relative tw-mt-1">
          <Combobox.Button as="div" className="tw-relative tw-w-full tw-cursor-default tw-overflow-hidden tw-rounded-lg tw-bg-white tw-text-left tw-shadow-md sm:tw-text-sm tw-flex hover:tw-bg-gray-100 focus-within:tw-bg-gray-100">
            ...
            <Combobox.Input
              ref={inputRef}
              className="tw-w-min tw-border-none tw-py-2 tw-pl-2 tw-pr-0 tw-text-sm tw-leading-5 tw-text-gray-900 focus:tw-outline-none tw-bg-transparent"
          ...
          >
            <Combobox.Options className="tw-absolute tw-mt-1 tw-max-h-60 tw-w-48 tw-overflow-auto tw-rounded-md tw-bg-white">
              <AssignedUserOption user={undefined} />
              {
                filteredUsers.map((user) => (
                  <AssignedUserOption user={user} />
                ))
              }
            </Combobox.Options>
          </Transition>
...
type AssignedUserOptionProps = {
  user: User | undefined;
};

const AssignedUserOption: React.FC<AssignedUserOptionProps> = (props) => {
  return (
    <Combobox.Option
      key={props.user ? props.user.id : -1}
      className={({ active }) =>
        `tw-relative tw-cursor-default tw-select-none tw-py-2 tw-px-4 ${active ? ' tw-bg-gray-100' : 'tw-text-gray-900'
        }`
      }
      value={props.user}
    >
      <span className="tw-block tw-truncate">
        {props.user ? props.user.first_name + ' ' + props.user.last_name : "Unassigned"}
      </span>
    </Combobox.Option >
  );
};