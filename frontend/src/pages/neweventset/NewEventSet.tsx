...
import { CreateSyncConfiguration, CreateSyncConfigurationRequest, DataConnection, GetSchema, GetSchemaRequest, GetSyncConfigurations, Schema } from "src/rpc/api";
...
type NewSyncConfigurationState = {
...
const INITIAL_STATE: NewSyncConfigurationState = {
...
const validateAll = (state: NewSyncConfigurationState): boolean => {
...
export const NewSyncConfiguration: React.FC<{ onComplete: () => void; }> = props => {
  const [state, setState] = useState<NewSyncConfigurationState>(INITIAL_STATE);
  const onBack = () => {
    switch (state.step) {
      case Step.One:
        props.onComplete();
    ...
    <>
      ...
      <div className="tw-flex tw-justify-center tw-h-full">
        <div className='tw-w-[400px] tw-pb-10 tw-px-8 tw-mx-auto tw-mt-24'>
          <NewSyncConfigurationForm state={state} setState={setState} onComplete={props.onComplete} />
        </div>
      </div>
    </>
  );
...
type NewSyncConfigurationFormProps = {
  state: NewSyncConfigurationState;
  setState: (state: NewSyncConfigurationState) => void;
};

export const NewSyncConfigurationForm: React.FC<NewSyncConfigurationFormProps & { onComplete: () => void; }> = props => {
  const state = props.state;
  const setState = props.setState;
  let stepElement;
  switch (state.step) {
    case Step.One:
      stepElement = <NewSyncConfigurationStepOne state={state} setState={setState} />;
      break;
    case Step.Two:
      stepElement = <NewSyncConfigurationStepTwo state={state} setState={setState} />;
      break;
    case Step.Three:
      stepElement = <NewSyncConfigurationStepThree onComplete={props.onComplete} state={state} setState={setState} />;
      break;
  }
...
export const NewSyncConfigurationStepOne: React.FC<NewSyncConfigurationFormProps> = props => {
  const state = props.state;
  const setState = props.setState;
  const onClick = () => {
...
export const NewSyncConfigurationStepTwo: React.FC<NewSyncConfigurationFormProps> = props => {
  const state = props.state;
  const setState = props.setState;
  const onClick = () => {
      ...
      <ConnectionSelector
        ...
        setConnection={(value: DataConnection) => {
          if (!state.connection || value.id !== state.connection.id) {
            setState({ ...state, connection: value, datasetName: undefined, tableName: undefined, });
          }
        }} />
      {
            ...
            <DatasetSelector
              ...
              setDatasetName={(value: string) => {
                if (value !== state.datasetName) {
                  setState({ ...state, datasetName: value, tableName: undefined, });
                }
              }}
            ...
            <TableSelector
              ...
              setTableName={(value: string) => {
                if (value !== state.tableName) {
                  setState({ ...state, tableName: value, });
                }
              }}
...
export const NewSyncConfigurationStepThree: React.FC<NewSyncConfigurationFormProps & { onComplete: () => void; }> = props => {
  const state = props.state;
  const setState = props.setState;
  const [loading, setLoading] = useState<boolean>(false);
  const [schemaLoading, setSchemaLoading] = useState<boolean>(false);
  const [schema, setSchema] = useState<Schema | null>(null);
  const [createSyncConfigurationSuccess, setCreateSyncConfigurationSuccess] = useState<boolean | null>(null);
  useEffect(() => {
  ...
  const createNewSyncConfiguration = async () => {
    setLoading(true);
    ...
    const payload: CreateSyncConfigurationRequest = {
    ...
    try {
      await sendRequest(CreateSyncConfiguration, payload);
      mutate({ GetSyncConfigurations }); // Tell SWRs to refetch event sets
      setCreateSyncConfigurationSuccess(true);
    } catch (e) {
      setCreateSyncConfigurationSuccess(false);
    }

    setLoading(false);
  };

  if (createSyncConfigurationSuccess) {
    return (
  ...
  return (
    <>
      ...
      <Button onClick={() => createNewSyncConfiguration()} className='tw-mt-5 tw-w-full tw-h-10'>{loading ? <Loading /> : "Save"}</Button>
    </>
  );
};