	...
	return (
		<>
			...
			<div className="tw-flex tw-flex-col tw-w-[900px] tw-py-12 tw-px-10 tw-mx-auto tw-mb-20 tw-mt-8 tw-bg-white tw-rounded-lg tw-shadow-md tw-items-center">
				<div className="tw-w-full tw-text-center tw-mb-5 tw-font-bold tw-text-lg">
					New Destination
				</div>
				{connectionType ? (
...
const NewDestinationConfiguration: React.FC<NewConnectionConfigurationProps> = (
	props
) => {
	const [state, setState] = useState<NewDestinationState>(
	...
	if (createConnectionSuccess) {
		return (
			<div>
				...
				<Button
					className="tw-block tw-mt-8 tw-mx-auto tw-mb-10 tw-w-32"
	...
	return (
		<div className="tw-w-[500px]">
			<div className="tw-flex tw-justify-center tw-items-center tw-mb-5">
				<img
					src={getConnectionTypeImg(props.connectionType)}
					alt="icon"
					className="tw-h-6 tw-mr-1.5"
				/>
				<div>
					Enter your {getConnectionType(props.connectionType)} configuration:
				</div>
			</div>
			<form onSubmit={createNewDestination}>
				...
				/>
				<FormButton className="tw-mt-5 tw-w-full tw-h-10">
				...
				{createConnectionSuccess !== null && (
					...
					<div className="tw-mt-3 tw-text-center">
						{createConnectionSuccess ? "Success!" : "Failure"}
					</div>
				)}
			</form>
		</div>
	...
	return (
		<Button
			className={mergeClasses(
				"tw-mt-8 tw-bg-slate-200 tw-text-slate-900 hover:tw-bg-slate-300 tw-border-slate-200 tw-w-full tw-h-10",
				testColor
			)}
		...
		<>
			...
			<div className="tw-flex tw-flex-row tw-gap-5 tw-flex-wrap tw-justify-center">
				<button
					...
					onClick={() => props.setConnectionType(ConnectionType.Snowflake)}
				>
					<img
						src={snowflake}
				...
				<button
					className={connectionButton}
					onClick={() => props.setConnectionType(ConnectionType.BigQuery)}
				>
					<img
						src={bigquery}
						alt="data source logo"
				...
				<button
					className={connectionButton}
					onClick={() => props.setConnectionType(ConnectionType.Redshift)}
				>
					<img
						src={redshift}
						alt="data source logo"
			...
			<div className="tw-flex tw-flex-row tw-mt-10">
				<button
					className={connectionButton}
					onClick={() => props.setConnectionType(ConnectionType.MongoDb)}
				>
					<img
						src={mongodb}
						alt="data source logo"
