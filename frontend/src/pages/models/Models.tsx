...
import { NewModel } from "src/pages/models/NewModel";
import { useModels } from "src/rpc/data";
...
export const Models: React.FC = () => {
  ...
  switch (step) {
    case Step.Initial:
      content = <ModelList setStep={setStep} />;
      break;
    case Step.NewModel:
      content = <NewModel onComplete={() => setStep(Step.Initial)} />;
      break;
    default:
      content = <></>;
  }
...
const ModelList: React.FC<{ setStep: (step: Step) => void; }> = ({ setStep }) => {
  const { models } = useModels();
  return (
    <>
      <div className="tw-flex tw-w-full tw-mb-5 tw-mt-2">
        <div className="tw-flex tw-flex-col tw-justify-end tw-font-bold tw-text-lg">Models</div>
        <Button className='tw-ml-auto tw-flex tw-justify-center tw-items-center' onClick={() => setStep(Step.NewModel)}>
        ...
        {models
          ...
          <table className="tw-min-w-full tw-border-spacing-0">
            ...
            <tbody>
              ...
              )) : <tr><td className={tableCellStyle}>No models yet!</td></tr>}
            </tbody>
          </table>
          :
