	...
	return (
		<div className="tw-w-full tw-px-28">
			<div className="tw-flex tw-flex-row tw-items-center">
				<span className="tw-text-2xl tw-font-semibold tw-text-slate-900">
					Your sources
				</span>
				<Tooltip
					placement="right"
					maxWidth="500px"
					label="These are the data sources you've setup previously."
				>
					<InfoIcon className="tw-ml-1.5 tw-h-3.5 tw-fill-slate-400" />
				</Tooltip>
				<Button
					className="tw-ml-auto tw-flex tw-flex-row tw-items-center tw-h-8 tw-bg-blue-600 hover:tw-bg-blue-800"
					onClick={() =>
						props.setState({ ...props.state, step: SyncSetupStep.Warehouse })
					}
				>
...
const SourceTable: React.FC<SetupSyncProps> = ({
	linkToken,
	state,
	setState,
}) => {
									...
									sources.map((source) => (
										...
										>
											<td className="tw-whitespace-nowrap tw-py-4 tw-pl-4 tw-pr-3 tw-text-sm tw-font-medium tw-text-slate-900 tw-flex tw-flex-row tw-items-center">
												<img
													className="tw-mr-2 tw-h-5"
													src={getConnectionTypeImg(
									...
									<tr>
										<td className="tw-whitespace-nowrap tw-pt-10 tw-pb-16 tw-pl-12 tw-pr-3 tw-text-sm tw-font-medium tw-text-slate-900 tw-flex tw-flex-row tw-items-center">
											...
											<div className="tw-flex tw-flex-col tw-ml-8">
												<span className="tw-text-lg tw-font-semibold tw-text-slate-500">
													Add a source
												</span>
												<span className="tw-text-slate-500">
													Start syncing your data by adding a source.
												</span>
												<Button
													className="tw-bg-blue-600 hover:tw-bg-blue-800 tw-w-32 tw-px-0 tw-mt-4"
													onClick={() =>
														setState({
															...state,
