...
export const App: React.FC = () => {
	...
	const [linkToken, setLinkToken] = useState<string | undefined>("test");
	...
	useEffect(() => {
		...
		if (needsInit) {
			window.addEventListener(
				"message",
				(message: MessageEvent<FabraMessage>) => {
	...
	return (
		<div className="tw-fixed tw-bg-[rgb(0,0,0,0.2)] tw-w-full tw-h-full">
			<div
				id="fabra-connect"
				className="tw-fixed tw-bg-white tw-flex tw-flex-col tw-w-[70%] tw-h-[75%] tw-top-[50%] tw-left-1/2 -tw-translate-y-1/2 -tw-translate-x-1/2 tw-rounded-lg tw-shadow-modal tw-items-center"
			>
...
const Header: React.FC<{ close: () => void; state: SetupSyncState }> = ({
	close,
	state,
}) => {
	return (
		<div className="tw-flex tw-flex-row tw-items-center tw-w-full tw-h-20 tw-min-h-[80px] tw-border-b tw-border-slate-200">
			<div className="tw-flex tw-flex-row tw-gap-10 tw-justify-center tw-items-center tw-w-full">
				<StepBreadcrumb
					...
					active={state.step <= SyncSetupStep.Warehouse}
					complete={state.step > SyncSetupStep.Warehouse}
				/>
				<StepBreadcrumb
					step={2}
					content="Connect source"
					active={state.step === SyncSetupStep.Connection}
					complete={state.step > SyncSetupStep.Connection}
				/>
				<StepBreadcrumb
					step={3}
					content="Define model"
					active={state.step === SyncSetupStep.Object}
					complete={state.step > SyncSetupStep.Object}
				/>
				<StepBreadcrumb
					step={4}
...
export const Footer: React.FC<FooterProps> = (props) => {
	...
	return (
		<div className="tw-flex tw-flex-row tw-w-full tw-h-20 tw-min-h-[80px] tw-border-t tw-border-slate-200 tw-mt-auto tw-items-center tw-px-28">
			{props.state.step > SyncSetupStep.Initial && (
				<button
					className="tw-border tw-border-slate-300 tw-font-medium tw-rounded-md tw-w-36 tw-h-10 tw-select-none hover:tw-bg-slate-100"
					onClick={props.back}
				>
					Back
				</button>
