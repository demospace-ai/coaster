          ...
          <table className="tw-min-w-full tw-divide-y tw-divide-slate-200">
            <thead className="tw-bg-slate-100">
              {syncs.length > 0 ?
                ...
                <tr>
                  <th scope="col" className="tw-py-3.5 tw-pl-4 tw-pr-3 tw-text-left tw-text-sm tw-font-semibold tw-text-slate-900" >Name</th>
                </tr>
              }
            </thead>
            <tbody className="tw-divide-y tw-divide-slate-200 tw-bg-white">
              {syncs.length > 0 ? syncs.map((sync) => {
                ...
                return (
                  <tr key={sync.id} className="tw-cursor-pointer hover:tw-bg-slate-50" onClick={() => navigate(`/sync/${sync.id}`)}>
