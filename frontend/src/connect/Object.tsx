...
export const ObjectSetup: React.FC<SetupSyncProps> = (props) => {
	...
	const setTableName = (tableName: string) =>
		props.setState({ ...props.state, tableName: tableName });
	const [limitPreview, setLimitPreview] = useState<boolean>(true);
	const [previewLoading, setPreviewLoading] = useState<boolean>(false);
	const [previewData, setPreviewData] = useState<ResultRow[] | undefined>(
		undefined
	);
	const [previewSchema, setPreviewSchema] = useState<Schema | undefined>(
		undefined
	);
	...
	const runQuery = async () => {
		...
		try {
			const response = await sendLinkTokenRequest(
				LinkGetPreview,
				props.linkToken,
				payload
			);
	...
	return (
		<div className="tw-w-full tw-px-28 tw-flex tw-flex-col">
			<div className="tw-mb-5 tw-text-2xl tw-font-semibold tw-text-slate-900">
				Define the data model to sync
			</div>
			<div className="tw-w-[50%] tw-min-w-[400px] tw-h-full">
				...
				<div className="tw-text-slate-600 tw-text-sm">
					This is the object that will be created from the data you define in
				...
				/>
				<div className="tw-text-base tw-font-medium tw-mt-8 tw-mb-1 tw-text-slate-800">
